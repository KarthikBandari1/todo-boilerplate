...
namespace todo3.EntityFrameworkCore.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly todo3DbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = todo3Consts.MultiTenancyEnabled ? null : (int?)MultiTenancyConsts.DefaultTenantId;
        ...
        public DefaultLanguagesCreator(todo3DbContext context)
        {
            _context = context;
        }
